    ...
    def load_from_disk(
        ...
        *,
        ...
        with open(path / "cfg.json", "r") as f:
            ...
            d_in = cfg_dict.pop("d_in", None) or cfg_dict.pop("d_model", None)
            if d_in is None:
                raise KeyError("cfg.json must contain 'd_in' or 'd_model'")
            # Map alternative key names to the expected SaeConfig fields
            if "n_dirs_local" in cfg_dict and "num_latents" not in cfg_dict:
                cfg_dict["num_latents"] = cfg_dict.pop("n_dirs_local")
            cfg = SaeConfig.from_dict(cfg_dict, drop_extra_fields=True)

        sae = Sae(d_in, cfg, device=device, decoder=decoder)
        safetensors_path = path / "sae.safetensors"
        pth_path = path / "state_dict.pth"
        if safetensors_path.exists():
            ...
            raw = torch.load(str(pth_path), map_location=device, weights_only=True)
            sd = raw.get("state_dict", raw)
            # Remap keys from the RIEBench/sdxl-unbox format
            key_map = {
                "pre_bias": "b_dec",
                "latent_bias": "encoder.bias",
                "decoder.weight": None,  # handled separately (transposed)
            }
            mapped = {}
            for k, v in sd.items():
                if k == "decoder.weight":
                    if decoder:
                        mapped["W_dec"] = v.T.contiguous()
                elif k in key_map:
                    mapped[key_map[k]] = v
                elif k != "stats_last_nonzero":
                    mapped[k] = v
            sae.load_state_dict(mapped, strict=decoder)
        else:
            raise FileNotFoundError(
                f"No weights file found at {path}: expected 'sae.safetensors' or 'state_dict.pth'"
            )
