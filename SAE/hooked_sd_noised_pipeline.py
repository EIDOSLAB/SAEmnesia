...
import torch
from diffusers import DDIMScheduler, DiffusionPipeline, StableDiffusionXLPipeline
...
class HookedDiffusionAbstractPipeline:
    ...
    def __init__(self, pipe: parent_cls, use_hooked_scheduler: bool = False, force_ddim_scheduler: bool = True):
        if use_hooked_scheduler:
            pipe.scheduler = HookedNoiseScheduler(pipe.scheduler)
        if force_ddim_scheduler:
    ...
    def _prepare_prompt(
        ...
        # NEW: Check if UNet expects added_cond_kwargs (SDXL-style)
        unet_config = self.pipe.unet.config
        requires_aesthetics_score = getattr(unet_config, "requires_aesthetics_score", False)
        ...
        if projection_class_embeddings_input_dim is not None:
            ...
            # Create time IDs (original_size, crops, target_size)
    ...
    def _postprocess_latents(self, latents, output_type, generator):
        if not output_type == "latent":
            # Scale latents
            scaled_latents = latents / self.pipe.vae.config.scaling_factor

            # SDXL VAE has force_upcast=True: must decode in float32 to avoid black/NaN images
            needs_upcasting = (
            ...
            if needs_upcasting:
                ...
                scaled_latents = scaled_latents.to(
                    next(iter(self.pipe.vae.post_quant_conv.parameters())).dtype
                )
            else:
                vae_dtype = self.pipe.vae.dtype
                if scaled_latents.dtype != vae_dtype:
                    scaled_latents = scaled_latents.to(vae_dtype)
...
class HookedStableDiffusionXLPipeline(HookedDiffusionAbstractPipeline):
    """
    Hooked version of Stable Diffusion XL Pipeline with support for activation caching.
    Inherits all functionality from HookedDiffusionAbstractPipeline.
    Does not force DDIM scheduler by default, preserving the native SDXL scheduler
    (required for distilled models like SDXL-Turbo).
    """
    parent_cls = StableDiffusionXLPipeline

    def __init__(self, pipe, use_hooked_scheduler: bool = False):
        super().__init__(pipe, use_hooked_scheduler=use_hooked_scheduler, force_ddim_scheduler=False)
    ...
    def _prepare_prompt(
        ...
        """
        Override _prepare_prompt to handle SDXL's dual text encoders and additional conditioning.
        """
        # 0. Default height and width to unet
        height = self.pipe.unet.config.sample_size * self.pipe.vae_scale_factor
        width = self.pipe.unet.config.sample_size * self.pipe.vae_scale_factor
        ...
        # 3. Encode prompt - SDXL uses both text encoders
        (
            prompt_embeds,
        ...
        # 7. Prepare added time ids & embeddings (SDXL-specific)
        ...
        if hasattr(self.pipe, 'text_encoder_2') and self.pipe.text_encoder_2 is not None:
            ...
            # Fallback: infer from pooled_prompt_embeds
        ...
        # Package SDXL-specific conditioning
        added_cond_kwargs = {
